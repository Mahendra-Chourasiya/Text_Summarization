...
# Ensure the API key is provided before initializing the model
...
if st.button("Summarize the Content from YT or Website"):
    ...
    if not groq_api_key.strip() or not generic_url.strip():
        ...
        st.error("Model is not initialized. Please check the API Key.")
    else:
        try:
            with st.spinner("Waiting..."):
                ...
                if not docs or len(docs) == 0:
                    ...
                    st.info(f"Extracted content: {docs[0].page_content[:500]}...")
